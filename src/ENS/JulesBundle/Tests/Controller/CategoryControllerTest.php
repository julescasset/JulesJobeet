...
class CategoryControllerTest extends WebTestCase
{
    public function testShow()
    {
        ...
        $this->assertEquals('Ens\JulesBundle\Controller\CategoryController::showAction', $client->getRequest()->attributes->get('_controller'));
        ...
        $crawler = $client->request('GET', '/');
        $link = $crawler->selectLink('20')->link();
        $crawler = $client->click($link);
        $this->assertEquals('Ens\JulesBundle\Controller\CategoryController::showAction', $client->getRequest()->attributes->get('_controller'));
        $this->assertEquals('programming', $client->getRequest()->attributes->get('slug'));
